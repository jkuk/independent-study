...
public class NFA {
	...
	public static NFA build(String postfixExpression) {
		...
		for (int i = 0; i < postfixExpression.length(); i++) {
			...
			if (character == Constants.ESCAPE_CHARACTER) {
				character = postfixExpression.charAt(++i);
	...
	private static NFA generateOperatorNFA(
		...
		if (operator == Constants.KLEENE) {
			return kleene(nfaStack.pop());
		}
		else if (operator == Constants.CONCATENATE) {
	...
	private static NFA kleene(NFA a) {
		...
		firstState.addTransition(Constants.EPSILON, a.getFirstState());
		firstState.addTransition(Constants.EPSILON, lastState);
		a.getLastState().addTransition(Constants.EPSILON, a.getFirstState());
		a.getLastState().addTransition(Constants.EPSILON, lastState);
	...
	private static NFA alternate(NFA b, NFA a) {
		...
		firstState.addTransition(Constants.EPSILON, a.getFirstState());
		firstState.addTransition(Constants.EPSILON, b.getFirstState());
		a.getLastState().addTransition(Constants.EPSILON, lastState);
		b.getLastState().addTransition(Constants.EPSILON, lastState);
