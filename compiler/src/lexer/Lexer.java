...
public class Lexer {
	...
	throws FileNotFoundException, IOException {
		...
		long finishTime = System.currentTimeMillis();
		Helper.print("ORIGINAL [" + (finishTime - startTime) + "ms]: " + regularExpression);

		startTime = System.currentTimeMillis();
		regularExpression = expandRanges(regularExpression);
		finishTime = System.currentTimeMillis();
		Helper.print("EXPANDED [" + (finishTime - startTime) + "ms]: " + regularExpression);

		startTime = System.currentTimeMillis();
		regularExpression = convertToPostfix(regularExpression);
		finishTime = System.currentTimeMillis();
		Helper.print("POSTFIX [" + (finishTime - startTime) + "ms]: " + regularExpression);
	...
	private String expandRange(String regularExpression, int i) {
		...
		expandedRange += (char)endCharacter + "" + LexerConstants.RIGHT_PARENTHESIS;
	...
	public Token getNextToken(CustomReader customReader) throws IOException {
		...
		for (int i = 0, lastValidIndex = 0; i < lexeme.length(); i++) {
			...
			if (dfaState.isAcceptingDFAState()) {
				// make accepting state a field set at dfa state creation
